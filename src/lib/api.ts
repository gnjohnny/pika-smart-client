...
export const getAllRecipes = async (queryKey: [string, RecipeQuery]) => {
  const [_key, { title, sortby, page = 1, limit = 3 }] = queryKey;
  try {
    const res = await axiosInstance.get("/recipe/all-recipes", {
